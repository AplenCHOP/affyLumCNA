# adjusted version
# test commands: 
# load("~/dataSets/AAML_0531/cnvData/lumData/GCadjusted/RData/773376.RData")
...
organize.ascat.segs <- function(ascat.output, markers) {
	...
	if (length(failed.names) == 0) {
		...
		# Format Ascat v.2.1: Annotate row numbers to chromosome and position
		...
		# out.seg[ , 1] <- as.character(out.seg[ , 1])
	 	# out.tmp <- out.seg
                out.seg$cn <- out.seg$nMajor + out.seg$nMinor
		...
		for (i in 1:(length(chromosome) - 1)) {
                ...
                out.c3$cn <- out.c3$nMajor + out.c3$nMinor
